...
class SaldoBodegaHandler2 extends Handler
{
    ...
    public function handle($context)
    {
        ...
        // Obtener el número de documento del contexto 
        ...
        if ($numeroDocumento && $tipoDocumentoERP) {
            // Obtener los detalles de cmdetgui que coinciden con el número de documento
            $detalles = cmdetgui::where('gui_numero', $numeroDocumento)
            ->where('gui_tipgui', $tipoDocumentoERP)
            ->get();
    
            // Verificar si hay detalles disponibles
            if ($detalles->isNotEmpty()) {
                // Iterar sobre los detalles y ejecutar la lógica de actualización
                ...
                Log::info('SaldoBodegaHandler', ['message' => 'No hay detalles disponibles para el número de documento: ' . $numeroDocumento]);
            }
        } else {
            Log::info('SaldoBodegaHandler', ['message' => 'Número de documento no encontrado en el contexto']);
        }
    }
    ...
    private function getBodegaModel($codigoProducto, $bodega)
    {
        try {
            ...
            Log::info('SaldoBodegaHandler', ['sql' => cmsalbod::where('bod_produc', $codigoProducto)->where('bod_bodega', $bodega)->where('bod_ano', Carbon::now()->format('Ym'))->toSql()]);

            $producto = cmsalbod::where('bod_produc', $codigoProducto)->where('bod_bodega', $bodega)->where('bod_ano', Carbon::now()->format('Y'))->first();

            if (!$producto) {
                $producto = $this->createBodega($codigoProducto, $bodega);
            }
    ...
    private function updateModelo($context, $detalle, &$isActual)
    {
        ...
        // Agregar log después de obtener el periodo
        ...
        // Agregar log antes de asignar el valor a $campoPeriodo
